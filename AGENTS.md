...
Phase 0, the deterministic primer-to-debrief vertical slice, the source-bounded GPT-5.6 layer, the four-zone 3D investigation through the Louis station, and Task 11's presentation-only media security contracts are implemented. The approved next build gate is Task 12 in the July 15 implementation plan: push-to-talk transcription and authorized speech playback. AI contract `1.1.0` and media contract `1.0.0` are active; signed speech authorization remains presentation-only and the existing typed/captioned path must stay complete when microphone or provider audio is unavailable.

The current 2D route is frozen as the complete, directly selectable non-spatial experience and as the DOM focus-overlay system used by the 3D route. Do not delete or weaken it while adding spatial presentation or media. Integrated baseline verification remains unfinished: a deliberately separate live-key model smoke test is still required when a key is available, and teacher-packet alignment plus remaining accessibility/reporting work remain later gates. Regular Playwright runs must remain no-key and deterministic.
